...
namespace testns {
	...
	template<typename L> using reverse = typename reverse_impl<L, typename create_empty<L>::f>::f;

	template<typename L> using reverse_test = std::is_same<L, reverse<reverse<L>>>;
}
